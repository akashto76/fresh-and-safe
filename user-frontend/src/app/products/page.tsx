...
const SkeletonCard = ({ isSingleColumn }: { isSingleColumn: boolean }) => (
  <div className="bg-white p-5 relative animate-pulse shadow-sm rounded-2xl">
    <div
...
export default function ProductsPage() {
  ...
  const toggleFavorite = (e: React.MouseEvent, id: string) => {
    ...
    setFavorites(prev => isAdding ? [...prev, id] : prev.filter(favId => favId !== id));
  ...
  return (
    <main className="min-h-screen bg-white text-slate-900 selection:bg-cyan-100 relative">
      ...
      >
        <div className="bg-slate-900/90 backdrop-blur-lg px-6 py-3 rounded-2xl shadow-2xl flex items-center gap-3">
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-8 pt-4 md:pt-6 pb-10">
        <div className="relative flex items-center justify-center mb-6 border-b border-slate-100 pb-4">
          ...
          {/* UPDATE: Sorting toggle is now hidden on desktop (md:hidden) */}
        ...
        {/* UPDATE: Grid logic
            - Mobile: respects isSingleColumn (grid-cols-1 or grid-cols-2)
            - Desktop: Always stays at 5 columns (md:grid-cols-3 lg:grid-cols-5)
        */}
        <div className={`grid gap-6 ${isSingleColumn ? 'grid-cols-1' : 'grid-cols-2'} md:grid-cols-3 lg:grid-cols-5`}>
          {loading
            ...
            : productList.map((item) => {
                ...
                return (
                  <div key={item.id} className="group bg-white p-5 transition-all duration-300 relative block">
                    <button
                      onClick={(e) => toggleFavorite(e, item.id)}
                      className="absolute top-7 right-7 z-20 p-2 bg-white/90 backdrop-blur-sm rounded-full shadow-sm transition-all active:scale-90 hover:scale-110"
                    >
                    ...
                    <Link href={`/products/${item.id}`}>
                      {/* UPDATE: Removed border from image container */}
